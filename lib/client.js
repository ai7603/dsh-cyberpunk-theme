	...
	factory: (require) => {
		...
		*/
		...
		const styles = { insert(css) {
			...
			if (sink === void 0) throw new Error("dsh-cyberpunk-theme: permanent style bridge is not installed (src/client/index.js must run first)");
			return sink.insert(css);
		} };
		const host = { call(method, args) {
			const bridge = permanentScope.__DSH_CYBERPUNK_HOST__;
			if (bridge === void 0) return Promise.reject(/* @__PURE__ */ new Error("dsh-cyberpunk-theme: permanent host bridge is not installed (src/client/index.js must run first)"));
			return bridge.call(method, args);
		} };
		function cyberpunkClientPlugin() {
			return { apply(ctx) {
				...
				function patch(p) {
					const accentChanged = p.accent !== void 0 && p.accent !== state.accent;
					const enabledChanged = p.enabled !== void 0 && p.enabled !== state.enabled;
					Object.assign(state, p);
					if (accentChanged || enabledChanged) syncTokens();
					if (p.enabled !== void 0 || p.perf !== void 0) syncEnabledAttrs();
					state.version += 1;
...
`;
				...
				function CyberOverlay() {
					...
					const children = [];
					if (st.grid) children.push(react.createElement("div", {
						key: "grid",
						className: "cp-layer cp-grid"
					}));
					if (st.scanlines) children.push(react.createElement("div", {
						key: "scan",
						className: "cp-layer cp-scanlines"
					}));
					children.push(react.createElement("div", {
						key: "vig",
						className: "cp-layer cp-vignette"
					}));
					if (st.glitch) children.push(react.createElement("div", {
						key: "glitch",
						className: "cp-layer cp-glitch"
					}));
					if (st.perf === "full" || st.perf === "balanced") {
						const edges = [
							...
							return react.createElement("span", {
								key: side,
								className: "cp-ribbon-edge cp-ribbon-edge--" + side
				...
				function StatusStrip() {
					...
					react.useEffect(function() {
						...
						let stopClock;
						let stopPing;
						if (timer !== void 0) stopClock = timer.interval(function() {
							setNow(/* @__PURE__ */ new Date());
						}, 1e3);
						function ping() {
							try {
								host.call("ping", {}).then(function() {
									setNet("online");
								}, function() {
									setNet("offline");
								});
						...
						ping();
						if (timer !== void 0) stopPing = timer.interval(ping, 5e3);
						return function() {
					...
					return react.createElement("div", { className: "cp-status" + (st.statusGlitch ? " cp-status--glitch" : "") }, react.createElement("span", { className: "cp-status-chip cp-status-time" }, "Local ", timeStr(now)), react.createElement("span", { className: "cp-status-chip" }, dateStr(now)), react.createElement("span", {
						className: "cp-status-chip cp-status-net",
						"data-net": net
					}, "Uplink ", react.createElement("span", {
						className: "cp-status-dot",
						"aria-hidden": "true"
					}, "●"), " ", netText), react.createElement("span", { className: "cp-status-chip" }, "Accent ", st.accent.toUpperCase()));
				}
				...
				function CyberSettings() {
					...
					return react.createElement("div", { className: "cp-settings" }, react.createElement("h2", { className: "cp-settings-title" }, "Cyberpunk 2077"), react.createElement("p", { className: "cp-settings-sub" }, "Night City reskin for the DSH shell. Dark mode shows the full neon look."), react.createElement("div", { className: "cp-group" }, react.createElement("div", { className: "cp-group-label" }, "Master switch"), react.createElement("label", { className: "cp-toggle cp-toggle--master" }, react.createElement("input", {
						type: "checkbox",
						checked: st.enabled,
						onChange: function(e) {
							patch({ enabled: e.target.checked });
						}
					}), react.createElement("span", null, "Enable Cyberpunk theme")), react.createElement("p", { className: "cp-note" }, st.enabled ? "Theme active. Turn it off to restore the native DSH look; this page stays available." : "Theme off — native DSH look restored. Turn it on to reapply the reskin.")), react.createElement("div", { className: optGroupClass }, react.createElement("div", { className: "cp-group-label" }, "Color scheme"), react.createElement("div", { className: "cp-row" }, schemeBtns), react.createElement("p", { className: "cp-note" }, "Current: ", react.createElement("strong", null, schemeLabel()), " (", activeScheme === "dark" ? "dark palette active" : "light palette active", ")")), react.createElement("div", { className: optGroupClass }, react.createElement("div", { className: "cp-group-label" }, "Accent"), react.createElement("div", { className: "cp-row" }, accentBtns)), react.createElement("div", { className: optGroupClass }, react.createElement("div", { className: "cp-group-label" }, "Performance"), react.createElement("div", { className: "cp-row" }, perfBtns), react.createElement("p", { className: "cp-note" }, "Full: all ambient motion + glitch. Balanced (default): slow stepped edge flow + static atmosphere + glitch flashes, cool. Eco: glitch flashes only, no static atmosphere or filters — coolest.")), react.createElement("div", { className: optGroupClass }, react.createElement("label", { className: "cp-toggle" }, react.createElement("input", {
						type: "checkbox",
						checked: st.grid,
						disabled: ambientDisabled,
						onChange: function(e) {
							patch({ grid: e.target.checked });
						}
					}), react.createElement("span", null, "Grid")), react.createElement("label", { className: "cp-toggle" }, react.createElement("input", {
						type: "checkbox",
						checked: st.scanlines,
						disabled: ambientDisabled,
						onChange: function(e) {
							patch({ scanlines: e.target.checked });
						}
					}), react.createElement("span", null, "Scanlines")), react.createElement("label", { className: "cp-toggle" }, react.createElement("input", {
						type: "checkbox",
						checked: st.glitch,
						disabled: !st.enabled,
						onChange: function(e) {
							patch({ glitch: e.target.checked });
						}
					}), react.createElement("span", null, "Screen glitch")), react.createElement("label", { className: "cp-toggle" }, react.createElement("input", {
						type: "checkbox",
						checked: st.statusGlitch,
						disabled: statusGlitchDisabled,
						onChange: function(e) {
							patch({ statusGlitch: e.target.checked });
						}
				...
				slots.inject("shell.overlay", function() {
					return slots.register({
						name: "shell.overlay",
						id: "cyberpunk-2077-overlay",
						order: -9999
					}, function() {
						return react.createElement(CyberOverlay);
					});
				});
				slots.inject("conversation.composer.dock", function() {
					return slots.register({
						...
						return react.createElement(StatusStrip);
					});
				});
				slots.inject("settings.section", function() {
					return slots.register({
						...
						return react.createElement(CyberSettings, props);
					});
				});
				function reactFiberOf(node) {
					const keys = Object.keys(node);
					for (let i = 0; i < keys.length; i++) if (keys[i].indexOf("__reactFiber$") === 0) return node[keys[i]];
					return null;
				}
				function usageFromStatsSpan(span) {
					let fiber = reactFiberOf(span);
					for (let hops = 0; fiber !== null && fiber !== void 0 && hops < 24; hops++) {
						let hook = fiber.memoizedState;
						while (hook !== null && hook !== void 0) {
							const value = hook.memoizedState;
							if (value !== null && typeof value === "object" && typeof value.cacheReadTokens === "number" && typeof value.uncachedInputTokens === "number" && typeof value.cacheWriteTokens === "number") return value;
							hook = hook.next;
						}
						fiber = fiber.return;
					}
				...
				if (typeof MutationObserver === "function") {
					...
					statsLineObserver.observe(document.body, {
						subtree: true,
						childList: true,
						characterData: true
					});
		...
		*/
		const PKG_ID$1 = "dsh-cyberpunk-theme";
		...
		*/
		function createPermanentStyles() {
			...
			return {
				insert(css) {
					...
					tag.dataset.plugin = PKG_ID$1;
					tag.dataset.pluginCss = "dsh-cyberpunk-theme/style-" + tags.size;
					tag.textContent = css;
		...
		*/
		function createPermanentHost() {
			return { call(method, args) {
				if (method === "ping") {
					if (!(!browserScope() || navigator.onLine !== false)) return Promise.reject(/* @__PURE__ */ new Error("dsh-cyberpunk-theme: browser is offline"));
				...
				return Promise.reject(/* @__PURE__ */ new Error("dsh-cyberpunk-theme: unknown host method " + String(method)));
			} };
		}
		//#endregion
